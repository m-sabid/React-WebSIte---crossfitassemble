          ...
          <SwiperSlide key={index}>
            <div className="image-slide-bgOne relative md:px-10 md:py-24 px-5 py-16 md:flex items-center justify-between">
              <div className="md:w-[70%] flex">
                <img
                  className="quote_image relative top-[-35px] quote_image_one inline-block"
                  src={quote}
                  alt=""
                />
                <div>
                  <span className="c-white italic mx-auto">
                    {item?.message}
                  </span>
                  <h2 className="c-white text-xs italic mx-auto">
                    {item?.days_ago} ago
                  </h2>
                </div>
                <img
                  className="quote_image quote_image_two inline-block align-bottom -mb-5"
                  src={quote}
                  alt=""
                />
              </div>
              <div className="md:w-[25%] lg:mr-[-35px] mt-8 md:mt-0 flex flex-col items-center justify-center">
                <div className="md:w-32 md:h-32 w-20 h-20 rounded-full overflow-hidden ring ring-warning ring-offset-base-100 ring-offset-2">
                  {item?.user?.profile_image ? (
                    ...
                    <img
                      className=" w-full h-full"
                      src="https://i.ibb.co/vHfKc6X/blank-profile-picture-g3bbbf5065-1280.png"
                      alt=""
                    />
                  )}
                </div>
                <h1 className="c-white md:text-2xl sm:text-xl font-bold pt-4">
                  {item?.user?.name}
                </h1>
                <p className="c-white text-xs">{item?.days_ago} ago</p>
              </div>
            </div>
          </SwiperSlide>
        ))}
